...
def load_and_train_model(model_root_path, old_model_path, fathomnet_root_path):
    ...
    final_model_path = os.path.join(checkpoint_folder, 'fn_final_trained_model_beit.pth')
    torch.save(model.state_dict(), final_model_path)

    # Save the final model to S3
    s3_final_model_path = os.path.join(S3_MODEL_ROOT_PATH, 'fn_final_trained_model_beit.pth')
...
final_model_path = os.path.join(current_working_dir, 'fn_trained_model_beit.pth')
