...
def load_and_train_model(model_root_path, old_model_path, fathomnet_root_path):
    ...
    num_classes = len(concepts)  # Set this to the number of your dataset classes
    model = timm.create_model(model_name, pretrained=True, num_classes=num_classes)

    # Load the pre-trained model parameters if available
    if old_model_path != "":
        model.load_state_dict(torch.load(old_model_path))
        print("Loaded the previous model parameters for further training")
